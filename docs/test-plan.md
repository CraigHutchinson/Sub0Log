...
| R1.3 no lock | `detail/atomics.hpp` picks 64-bit atomics where lock-free and a split 32-bit protocol (identical bytes) where not, and `static_assert`s that 32-bit atomics are lock-free, so a target that would silently substitute a lock table fails the build; `unit/atomics.test.cpp` checks byte identity, bounded and concurrent claims and publish ordering on both paths; CI `linux-gcc-split-atomics` runs every suite on the split path and `embedded-cortex-m` runs it on Cortex-M3/M4/M7/M33 under QEMU; design (one `fetch_add` claim) + bench `claim`, `throughput` scaling 1→4 threads; stress `oversubscribe` holds the accounting invariant at 4x cores, and ThreadSanitizer over every stress scenario reported no race |
...
| R8.1 Qt consumers (issue #1) | `qt_keywords::*` (label `qt`): every public header plus a fixed, a continuation-chained and a C-ABI record written and decoded, with Qt's `emit`/`slots`/`signals`/`foreach`/`forever` macros defined before *and* after Sub0Log -- against real Qt 6 on linux-gcc, linux-clang and windows-msvc (`SUB0LOG_TEST_QT=ON`, so a missing Qt fails rather than skips), and against a Qt-less shim of the same macros everywhere else. Both orders were confirmed to fail on the pre-rename headers |
| Issue #2 in-memory producer | integration `memory_segment.test.cpp`: typed round trip through a caller buffer, a previous run's records not reappearing (fails with the zeroing removed), a three-chunk buffer asserting decoded + dropped == emitted, refusal of misaligned/undersized/empty storage, single ownership across a move, `usage()` clamped at the chunk total, the post-mortem read-then-recreate order, an A/B rotation drained to a stand-in sink losing nothing over dozens of rotations, and the 192-byte compact header read back by the ordinary reader (an unmodified pre-change `sub0log-cat` was also checked to read such an image). `embedded::producer` is built `-fno-exceptions -fno-rtti` and counts global `operator new` across creation and ~2000 records (must be zero; an injected `new` fails it); `embedded::decode` runs `sub0log-cat` on its dump. `embedded::freestanding_probe` runs `SUB0LOG_PLATFORM_CUSTOM` on the host; CI's `embedded-arm` cross-links the same file for bare-metal Cortex-A7 and prints its size (`docs/embedded.md`) |
| Issue #2 Android lifecycle | CI `android-lifecycle`: `tests/android/lifecycle.cpp` (NativeActivity) on an API 34 emulator through launch, pause, resume and `am force-stop`, twice; `check_lifecycle.sh` asserts both file segments decode with nothing undecodable, the lifecycle is recorded, no DESTROY/exit record exists after a kill, and the file segment holds ticks past the last in-memory hand-off |
| Issue #2 rotation/hand-off | example ctest `13_rotation`: 21 rotations (watermark and a lifecycle "pause"), every one of 3000 events merged back exactly once, 0 dropped; retention keep=3 bounds the outbox to 3 |
| libc++ 18 (NDK) | CI `linux-clang-libcxx`: the whole suite on libc++ 18, which lacks `std::atomic_ref`, so `detail::AtomicRef`'s builtin fallback is what runs |
| 32-bit ARM (armeabi-v7a ISA) | CI `linux-armv7-qemu`: the whole suite cross-built for armv7-a and run under qemu-arm (the 3 tests that exec `sub0log-cat` as a subprocess excluded). Its publish-ordering test is what found the reader's non-atomic 8-byte head-word load |
| libatomic | root `CMakeLists.txt` link-probes the library's atomic operations and links `atomic` only if needed (fallback verified on `-m32 -march=i386`); `android-lifecycle` builds all four ABIs and rejects any `.so` with unresolved `__atomic_*` symbols |
| Chunk-header CRC-32 | `unit/wire.test.cpp`: the standard check vector, chaining, and the nibble table against an independent bit-by-bit reference on 2,000 inputs; `integration/reader.test.cpp`'s exhaustive single-byte corruption sweep (integrated from `claude/sub0log-cpp26-architecture-hc9fzu`), which also passes on the split path |
