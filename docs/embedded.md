...
|---|---|---|
| static-memory producer path: no heap allocation, no OS mapping call | **met**, measured | `tests/embedded/producer.cpp` replaces global `operator new` and counts across `createInMemory` + ~2000 records; the count must be zero. A deliberately injected `new` fails it. The segment path is empty -- there is no file. The emit path and creation are counted separately. Under MSVC's iterator debugging (Debug builds), each `std::string` in `Options` and the segment path allocates a debug container proxy at creation; that count is reported rather than failed, because it is the STL's checking, not the producer (Release: zero). The emit path must be zero everywhere. |
