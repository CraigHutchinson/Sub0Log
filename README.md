...
The tests and coverage badges are generated by the `coverage` CI job
(`.github/workflows/ci.yml`) from an actual `--coverage` build and the same
`ctest` run that badge claims to summarise, published to this repository's
`badges` branch on every push to `main` -- not hand-set numbers, and not a
third-party service with its own account or token to go stale. They read
"unknown" only before that job has run once on `main`.
...
(`REQUIREMENTS.md` R8.3). Two supported ways in, both exercised end to end by
`tests/packaging` rather than only described here:
...
`tests/packaging/consumer_cpm` is exactly this block, driven by
`tests/packaging/drive_cpm.cmake` as a ctest (`packaging::cpm_fetch`) against
a real `CPMAddPackage(GIT_REPOSITORY ...)` fetch -- not a claim that it works,
a test that it does.
...
Proven the same way, against a real `cmake --install` into a throwaway
prefix, by `packaging::find_package`.
...
adopting one of them the better answer instead of building this. It is
where the comparisons live; this file mostly avoids making them, on the
theory that "look at what this one does" is the fairer pitch and "here is
what we checked before deciding not to" is the fairer place for the rest.
